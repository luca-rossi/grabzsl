...
class TrainerTfvaegan():
	...
	def __init__(self, data, dataset_name, n_features=2048, n_attributes=85, latent_size=85, features_per_class=1800,
				batch_size=64, hidden_size=4096, n_epochs=30, n_classes=50, n_critic_loops=5, n_feedback_loops=2,
		...
		self.n_feedback_loops = n_feedback_loops
		self.lr_cls = lr_cls
	...
	def __train_epoch(self, epoch):
		'''
		Train the models for one epoch. Each epoch consists of n_feedback_loops feedback loops.
		During each feedback loop, the critic and decoder are trained for n_critic_loops steps,
		then the generator is trained for one step with the encoder, the feedback module, and optionally the decoder.
		'''
		for loop in range(0, self.n_feedback_loops):
			for i in range(0, self.data.dataset_size, self.batch_size):
				...
				for p in self.model_decoder.parameters(): #unfreeze decoder
					p.requires_grad = True
				# train the decoder and critic for n_critic_loops steps, dynamically adjust weight_gp
				gp_sum = 0
				for _ in range(self.n_critic_loops):
					loss_critic, wasserstein, gp_incr = self.__decoder_critic_step(loop)
					gp_sum += gp_incr
				gp_sum /= (self.weight_critic * self.weight_gp * self.n_critic_loops)
				...
				if self.verbose and i % (self.batch_size * 5) == 0:
					print('%d/%d - %d/%d' % (loop + 1, self.n_feedback_loops, i, self.data.dataset_size))
